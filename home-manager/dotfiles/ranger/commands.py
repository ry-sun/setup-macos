...
class chmod(Command):
    """
    :chmod:
    Prompts for chmod mode and applies it to the currently selected file.
    """

    def execute(self):
        # Get the currently selected file's path
        file = self.fm.thisfile.path
        # Prompt the user to input the mode number for chmod
        mode = self.rest(1)

        if mode:  # If user provides input
            # Execute the chmod command with the user-provided mode
            self.fm.execute_console(f"shell chmod {mode} '{file}'")
        else:
            # If no mode was provided, show an error
            self.fm.notify("No mode entered!", bad=True)
